...
@app.post("/api/scheduler/run-now", tags=['Пользователь'])
...
def custom_openapi():
    ...
    openapi_schema = get_openapi(
        ...
        В данном разделе представлена документация к ресурсам REST API, реализованного с помощью Python3 Fastapi, для внутреннего функционирования веб-сервиса Intranet2.0!

        Особенности проекта:
            - Модульная структура
            - Асинхронность
            - Взаимодействие 2х Баз Данных
        """,
        routes=app.routes,
