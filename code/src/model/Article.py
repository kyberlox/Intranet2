...
class Article:
    ...
    async def add(self, article_data, session):
        return await ArticleModel().add_article(article_data=await self.make_valid_article(article_data, session), session=session)
                    ...
                    if 'likes_from_b24' in inf['indirect_data'] and inf['indirect_data']['likes_from_b24'] is not None: 
                        for user_id in inf['indirect_data']['likes_from_b24']:
                            ...
                            if isinstance(user_exist, types.CoroutineType) or user_exist is None:
                                ...
                                has_usr_liked = await LikesModel(user_id=int(user_id), art_id=int(inf['id'])).has_liked(session=session)
                                if has_usr_liked['likes']['likedByMe']:
                                    continue
                                else:
                                    await LikesModel(user_id=int(user_id), art_id=int(inf['id'])).add_or_remove_like(session=session)
                    ...
                    if likes_info != "Not found" and 'VOTES' in likes_info.keys():
                        for vote in likes_info['VOTES']:
                            ...
                            if isinstance(user_exist, types.CoroutineType) or user_exist is None:
                                ...
                                await LikesModel(user_id=vote['USER_ID'], art_id=inf['id']).add_like_from_b24(vote['CREATED_'], session=session)
...
async def get_articles_by_tag_id(section_id: int, tag_id: int, request: Request, session: AsyncSession=Depends(get_async_db)):
    session_id = ""
    token = request.cookies.get("Authorization")
    if token is None:
        token = request.headers.get("Authorization")
