...
class User:
    ...
    async def anniversary_in_company(self, session):
        ...
        ID = 7 - это +год рабоыты в компании
        ID = 8 - это Юбилей 5 лет работы в компании
        ID = 9 - это Юбилей 10 лет работы в компании
        ID = 10 - это +год рабоыты в компании
        ID = 11 - это Юбилей 5 лет работы в компании
        ID = 12 - это Юбилей 10 лет работы в компании
        ID = 13 - это Юбилей 10 лет работы в компании
        ID = 14 - это Юбилей 10 лет работы в компании
        ID = 15 - это Юбилей 10 лет работы в компании
        """
        try:
            from ..services.Peer import Peer
            from datetime import datetime
            LAUNCH_DATE_OF_CAPITAL_EMK = datetime.strptime("2026-02-03", '%Y-%m-%d')
            YEARS_ID = {'1': 7, "5": 8, "10": 9, "15": 11, "20": 12, "25": 13, "30": 14, "35": 15, "40": 16} # менять значеняи к годам если поменялись айдишники
            ...
            for user in all_users:
                if user.active is True:
                    ...
                    if ('date_register' in user.indirect_data and user.indirect_data['date_register'] != '') or ("date_of_employment" in user.indirect_data and user.indirect_data['date_of_employment'] != ''):
                        ...
                        # test_today = datetime.strptime('22.03.2026', '%d.%m.%Y')
            ...
            LogsMaker().warning_message(f'Ошибка в назначении баллов за годовщину:{e}')
            return False
...
async def get_new_users_ids(session: AsyncSession = Depends(get_async_db)):
    ...
    stmt = text("""
        SELECT * FROM users 
        WHERE indirect_data->>'date_of_employment' IS NOT NULL
          AND indirect_data->>'date_of_employment' != ''
          AND (
            CASE 
                WHEN indirect_data->>'date_of_employment' ~ '^\d{4}-\d{2}-\d{2}' THEN 
                    (indirect_data->>'date_of_employment')::date
                WHEN indirect_data->>'date_of_employment' ~ '^\d{2}\.\d{2}\.\d{4}' THEN 
                    TO_DATE(indirect_data->>'date_of_employment', 'DD.MM.YYYY')
                ELSE NULL
            END
          ) >= :start_date
    """)
    
    result = await session.execute(stmt, {"start_date": DATE_START_MERCH})
