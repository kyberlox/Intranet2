...
async def regconf(request: Request, session_data: Dict[str, Any] = Depends(get_current_session), response: Response = None):
    ...
    cookies = { 'session_id': session_data["session_id"]}
    res = requests.post(url='https://gpt.emk.ru/login', cookies=cookies)
...
async def tepconf(request: Request, session_data: Dict[str, Any] = Depends(get_current_session), response: Response = None, sess: AsyncSession = Depends(get_async_db)):
    ...
    session_id = session_data["session_id"]
    position = session_data['user_info']['WORK_POSITION']
    response = RedirectResponse(url=f"com.exhibitions://auth?id={ID}&session_id={session_id}&fio={fio}&department={department}&position={position}", status_code=302)
