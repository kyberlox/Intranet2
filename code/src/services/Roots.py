...
async def create_editor_moder(user_uuid: int, sec_id: int, request: Request, session: AsyncSession=Depends(get_async_db)):
...
async def delete_editor_moder(user_uuid: int, sec_id: int, request: Request, session: AsyncSession=Depends(get_async_db)):
    user_id = await get_uuid_from_request(request, session=session)
    editor_roots = await get_editor_roots(user_id, session=session)
    if "EditorAdmin" in editor_roots.keys() and editor_roots["EditorAdmin"] == True:
