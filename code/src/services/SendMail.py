...
class SendEmail:
    ...
    def send_congratulations(self):
        try:
            ...
            self.file_url = self.data['file_url'] # file_id[-1] 
    ...
    def send_active_purchase(self):
        try:
            ...
            msg = MIMEMultipart() 
            msg["From"] = self.data['sender']
            msg["To"] = f'{server_mail_login}, merch.store@emk.ru'
    ...
    def send_to_birthday_notifications(self):
        try:
            ...
            text_msg = f'''<p>Дорогой(ая) коллега!</p>\
                        <p>С днём рождения! </p>\
                        <p>Пусть новый год жизни приносит вдохновение в работе, радость в мелочах и
                        поводы для гордости. Желаем энергии для смелых решений, лёгкости в трудные
                        дни и людей рядом, которые делают путь интереснее.</p>\
    ...
    def send_to_anniversary_in_company(self, year):
        try:
            ...
            text_msg = f'''<p>Дорогой(ая) коллега!</p>\
                        ...
                        <p>Желаем, чтобы работа продолжала приносить удовольствие, а каждый новый год  
                        в компании открывал интересные возможности.</p>\
