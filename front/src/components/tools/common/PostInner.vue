<template>
  <div class="news-inner__page__wrapper mt20">
    <div
      v-if="currentPost && (type == 'default' || type == 'adminPreview')"
      class="row row-gap-50"
    >
      <div
        v-if="currentPost.section_id !== 32 && sectionId !== '32'"
        class="col-12 col-lg-6 mb-2 pos-rel"
      >
        <SwiperBlank
          :videosNative="currentPost?.videos_native"
      ...
      <div
        :class="{
          'col-12 col-lg-6': currentPost.section_id !== 32 && sectionId !== '32',
        }"
      >
        <div class="news__detail__content">
          <div v-if="currentPost.name" class="news__detail__top">
            <div class="row mb-2">
              <div class="col-12">
          ...
          >
            <div v-if="currentPost.reactions" class="news__detail__like-wrapper">
              <Reactions
          ...
          <div
            v-if="currentPost.indirect_data && 'tags' in currentPost.indirect_data"
            class="tags"
          >
            <div
              v-for="(tag) in (currentPost.indirect_data.tags as ITag[])"
              :key="tag.id"
              class="tag__wrapper"
            >
              <div
                class="tags__tag tags__tag--inner section__item__link btn-air"
          ...
          <div
            v-if="currentPost.indirect_data?.phone_number"
          ...
          <div
            v-if="
              ...
              !String(currentPost.content_text).includes('undefined')
            "
            class="news__detail__discr"
            v-html="currentPost.content_text.replaceAll('&nbsp;', ' ')"
          ></div>
          <div v-if="currentPost.documentation" class="news__detail__documents">
            <div
              class="news__detail__document"
              v-for="(doc, index) in currentPost.documentation"
              :key="'doc' + index"
            >
              <a class="news__detail__document__link" :href="String(doc.file_url)" _blank
                >Открыть {{ doc.original_name }}
                <DocIcon />
              </a>
            </div>
          </div>
          ...
          <div
            class="news__about-event__wrapper"
            v-if="currentPost.indirect_data?.bx_event?.ID && currentPost.section_id == 53"
          >
            <a
              :href="`https://portal.emk.ru/calendar/?EVENT_ID=${currentPost.indirect_data.bx_event.ID}`"
      ...
      <div v-if="currentPost.section_id == 32 || sectionId == '32'" class="row mb-5">
        <div
          :to="{ name: 'user', params: { id: user.id } }"
          v-for="user in previewElement
            ? currentPost.users
            : currentPost.indirect_data?.users"
          :key="user.id"
          class=""
        >
          <RouterLink
          ...
          >
            <figure>
              <img
                class="img-fluid img-thumbnail news__person__img"
