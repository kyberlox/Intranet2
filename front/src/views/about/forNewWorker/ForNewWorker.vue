  ...
  <div class="memo__section row">
    ...
    <div class="col-12 col-lg-12 col-xl-9 col-xxl-9">
      <div class="memo__items">
        <div class="memo__item__group" id="memo__items__">
          ...
          <div class="memo__item" v-if="endingSlide(pageContent)">
    ...
    <div class="d-none d-lg-block col-xl-3 col-xxl-3">
      <ul class="memo__menu-dots sticky__menu">
        <li
          v-for="item in filterContent(pageContent).filter((e) => e.name)"
          :key="'mark' + item.id"
          class="memo__menu-dot"
        >
          <div @click="navigate(item.id)" class="memo__menu-dot__item">
            {{ item.name }}
          </div>
        </li>
      </ul>
  ...
  setup() {
    ...
    onMounted(async () => {
      const res = await Api.get(
        `article/find_by/${sectionTips["НовомуСотруднику"]}`,
        null,
      ...
      res.forEach((e: IForNewWorker) => {
        if (
          e.indirect_data &&
          "module" in e.indirect_data &&
          e.indirect_data.module &&
          !locations.value.includes(e.indirect_data.module) &&
          e.indirect_data.module !== "Заключение"
        ) {
      ...
      return pageContent?.filter(
        (e) =>
          ...
          (!activeLocation.value
            ? !e.indirect_data?.module
            : e.indirect_data?.module == activeLocation.value)
      );
    };
    ...
    return {
      ...
      showLocations,
      locations,
      endingSlide: (pageContent: IForNewWorker[]) =>
        pageContent.find(
          (e: IForNewWorker) =>
            activeLocation.value &&
            e.indirect_data &&
